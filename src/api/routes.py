...
from src.api.deps import get_current_user, get_chat_service
...
async def chat_stream(prompt: str,chat_id : UUID, user = Depends(get_current_user), chat_servise = Depends(get_chat_service)):
    context = await chat_servise.get_chat_context(user.id, chat_id)
    context+= prompt
    async def generate():
